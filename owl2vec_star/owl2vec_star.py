...
def __perform_ontology_embedding(config):
    ...
    if 'pre_train_model' not in config['MODEL'] or not os.path.exists(config['MODEL']['pre_train_model']):
        ...
        model_ = gensim.models.FastText(all_doc, vector_size=int(config['MODEL']['embed_size']),
        ...
        model_ = gensim.models.FastText.load(config['MODEL']['pre_train_model'])
        if len(all_doc) > 0:
...
def __perform_joint_ontology_embedding(config):
    ...
    if 'pre_train_model' not in config['MODEL'] or not os.path.exists(config['MODEL']['pre_train_model']):
        ...
        model_ = gensim.models.FastText(all_doc, size=int(config['MODEL']['embed_size']),
                                        window=int(config['MODEL']['window']),
                                        workers=multiprocessing.cpu_count(),
